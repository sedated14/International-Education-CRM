                    ...
                    {lead.agencyProfile && (
                        <div className="flex items-center gap-1.5 text-[11px] text-gray-600 mt-1.5 bg-white/60 px-2 py-1 rounded-lg w-fit border border-yellow-100/50 backdrop-blur-sm">
                            ...
                            <div className="flex items-center gap-1.5 font-bold">
                                ...
                                <span className="text-gray-500 font-medium">{lead.agencyProfile.country}</span>
                            </div>
                        </div>
                    )}
                </div>
            ...
            <div className="p-5 pt-4">
                ...
                <div className="grid grid-cols-2 gap-2 mb-4 bg-gray-50 p-3 rounded-xl border border-gray-100">
                    ...
                    <div className="flex flex-col gap-0.5">
                        ...
                        <span className="font-bold text-sm text-blue-900">{studentProfile.gradeApplyingTo || '?'}</span>
                    </div>
                    ...
                    {(studentProfile.seekingGraduation || studentProfile.graduatedInHomeCountry) && (
                        <div className="col-span-2 flex gap-2 mt-1">
                            {studentProfile.seekingGraduation && (
                                <span className="px-2 py-1 bg-indigo-100 text-indigo-700 rounded text-[10px] font-bold uppercase tracking-wider flex-1 text-center">
                                    Seeking Graduation
                                </span>
                            )}
                ...
                <div className="flex flex-wrap gap-1.5 mb-3">
                    {studentProfile.sports?.slice(0, 2).map((sport, i) => (
                        <span key={i} className="text-[10px] font-bold bg-gray-50 text-gray-500 px-2 py-1 rounded-md flex items-center gap-1">
                ...
                <div className="flex justify-between items-center text-xs text-gray-400 font-bold uppercase tracking-wider pt-3 border-t border-gray-50">
                    <span>{lead.timestamp}</span>
                    <div className="flex gap-1">
                        {[...Array(5)].map((_, i) => (
                            <div key={i} className={`w-1.5 h-1.5 rounded-full ${i < lead.score ? 'bg-orange-500' : 'bg-gray-200'}`} />
                        ))}
                    </div>
                </div>
                ...
                <div className="mt-4 pt-4 border-t border-gray-100 grid grid-cols-1 gap-2">
                    ...
                    <div className="flex justify-between items-center text-[10px] bg-blue-50 -mx-4 px-4 py-2 mt-1">
