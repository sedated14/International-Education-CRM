        ...
        <div className="flex-1 grid grid-cols-4 gap-6 min-h-0 pb-2">
          ...
          <LeadColumn title="Follow Up" icon={<Clock size={20} />}>
            {[...activeLeads]
              ...
              .map(lead => (
                <div key={lead.id} onClick={() => setSelectedLeadId(lead.id)} className={`bg-white p-4 rounded-[20px] border shadow-sm hover:shadow-md cursor-pointer transition-all group mb-2 ${lead.type === 'Student' ? 'border-amber-200 hover:border-amber-300' : 'border-blue-200 hover:border-blue-300'}`}>
                  <div className="flex justify-between items-center mb-2">
                    <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded ${lead.type === 'Student' ? 'bg-amber-50 text-amber-600' : 'bg-blue-50 text-blue-600'}`}>{lead.type}</span>
                    <span className={`text-[10px] font-bold ${lead.type === 'Student' ? 'text-amber-500' : 'text-blue-500'}`}>
                      {lead.followUpDate
                  ...
                  <button className={`w-full py-2 rounded-lg text-xs font-bold transition-colors ${lead.type === 'Student' ? 'bg-amber-50 text-amber-600 hover:bg-amber-100' : 'bg-blue-50 text-blue-600 hover:bg-blue-100'}`}>
                    Mark Complete
                  </button>
                </div>
              ))}
