            ...
            <main className="flex-1 flex flex-col h-screen overflow-hidden">
                ...
                <header className="bg-white border-b border-gray-200 px-8 py-6 flex justify-between items-center z-20">
                    <div>
                        <div className="flex items-center gap-3 mb-1">
                            ...
                            <h1 className="text-2xl font-black text-gray-900">Student Leads</h1>
                        </div>
                        <p className="text-gray-500 font-medium text-sm ml-12">Manage student inquiries and applications</p>
                    </div>
                    <div className="flex gap-4">
                        {selectedIds.size > 0 && (
                            <>
                                <button
                                    ...
                                    className="flex items-center gap-2 px-5 py-2.5 bg-gray-100 border border-gray-200 text-gray-700 font-bold rounded-xl hover:bg-gray-200 transition-all text-sm animate-in fade-in slide-in-from-top-2"
                                >
                                    <Archive size={16} />
                        ...
                        <button className="flex items-center gap-2 px-5 py-2.5 bg-white border border-gray-200 text-gray-700 font-bold rounded-xl hover:bg-gray-50 transition-all text-sm disabled:opacity-50 disabled:cursor-not-allowed">
                            <Download size={16} />
                            Export CSV
                        </button>
                        <Link href="/apply">
                ...
                <div className="flex-1 overflow-auto p-8">
                    ...
                    <div className="flex gap-4 mb-8">
                        ...
                        <div className="flex bg-white rounded-2xl p-1 shadow-sm overflow-x-auto custom-scrollbar">
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-gray-50 border-b border-gray-100 text-xs uppercase tracking-wider text-gray-400 font-bold cursor-pointer select-none">
                                    ...
                                    <th className="px-6 py-5 hover:text-gray-600 transition-colors" onClick={() => handleSort('country')}>
                                        Country {sortConfig?.key === 'country' && (sortConfig.direction === 'asc' ? '↑' : '↓')}
                                    </th>
                                    <th className="px-6 py-5 hover:text-gray-600 transition-colors" onClick={() => handleSort('status')}>
                                        Status {sortConfig?.key === 'status' && (sortConfig.direction === 'asc' ? '↑' : '↓')}
                                    </th>
                                    <th className="px-6 py-5 hover:text-gray-600 transition-colors" onClick={() => handleSort('program')}>
                                        Program Interest {sortConfig?.key === 'program' && (sortConfig.direction === 'asc' ? '↑' : '↓')}
                                    </th>
                                    <th className="px-6 py-5 text-right hover:text-gray-600 transition-colors" onClick={() => handleSort('source')}>
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {sortedStudents.map(student => (
                                    <tr key={student.id} className={`group hover:bg-blue-50/50 transition-colors ${selectedIds.has(String(student.id)) ? 'bg-blue-50/30' : ''}`}>
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <Link
                                                        ...
                                                        className="font-bold text-gray-900 hover:text-blue-600 transition-colors cursor-pointer text-base block"
                                                    >
                                                        {student.name}
                                                    </Link>
                                        ...
                                        <td className="px-6 py-5">
                                            <div className="text-sm font-bold text-gray-900">{student.program}</div>
                                            <div className="text-xs text-gray-400">{student.duration}</div>
                                        </td>
                                        <td className="px-6 py-5 text-right">
                                            <div className="text-xs font-bold uppercase tracking-wide text-gray-500 bg-gray-100 px-2 py-1 rounded-md inline-block">
                                                {student.source}
                                            </div>
                                            {student.agencyName && (
                                                <div className="text-xs mt-1 font-bold text-blue-600">
                                        ...
                                        <td className="px-8 py-5 text-right">
                                            <button
                                                ...
                                                className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white border-2 border-gray-100 hover:border-black hover:bg-black hover:text-white transition-all text-gray-400"
                                            >
...
const StatusBadge = ({ status }: { status: string }) => {
    ...
    return (
        <span className={`px-3 py-1.5 rounded-lg text-xs font-black uppercase tracking-wide ${styles[status] || 'bg-gray-100 text-gray-500'}`}>
            {status}
        </span>
    );
};
